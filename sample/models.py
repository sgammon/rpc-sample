...
# canteen
from sample.base import Model, model
...
def fixtures():
  ...
  # make our fixture sentinel and check it
  fixture_key = model.Key(Fixtures, 'state')
  fixture_sentinel = Fixtures.get(fixture_key)
  if not fixture_sentinel or not fixture_sentinel.done:
    ...
    if not failures:
      Fixtures(key=fixture_key, done=True).put()
      print "Storing fixture sentinel at key: '%s'." % fixture_key.urlsafe()
